---
title: "Reference Library"
summary: "Curated cheat sheets, books, writeups, videos, labs, and external tools."
status: "reviewed"
last_reviewed: "2026-06-06"
tags:
  - references
related:
  - curated-cheat-sheets.md
references: []
---
# Reference Library

## Purpose

This section preserves useful external material without turning field guides into link dumps.

## Pages

- [Root Resource Listing](../RESOURCES.md)
- [Clean Resource Map](../RESOURCE-MAP.md)
- [Curated Cheat Sheets](curated-cheat-sheets.md)
- [Books and Courses](books-and-courses.md)
- [Blogs and Writeups](blogs-and-writeups.md)
- [Videos and Talks](videos-and-talks.md)
- [Labs and Practice](labs-and-practice.md)
- [External Tools](external-tools.md)
- [All Source Resources](all-source-resources.md)

## Category Resource Indexes

- [API, Auth, OAuth, and GraphQL](resource-indexes/api-auth-graphql.md)
- [Blogs, Writeups, GitBooks, and Bug Bounty References](resource-indexes/blogs-writeups-gitbooks.md)
- [Burp Suite and Tooling](resource-indexes/burp-tooling.md)
- [Cloud and Infrastructure](resource-indexes/cloud-infra.md)
- [File Upload and Parser Abuse](resource-indexes/file-upload-parser-abuse.md)
- [Miscellaneous Unsorted Source Links](resource-indexes/misc-uncategorized.md)
- [Payloads, Cheat Sheets, and Wordlists](resource-indexes/payloads-cheatsheets-wordlists.md)
- [Recon and OSINT](resource-indexes/recon-osint.md)
- [RCE, CVE, and Research](resource-indexes/rce-cve-research.md)
- [SQL Injection](resource-indexes/sql-injection.md)
- [SSRF](resource-indexes/ssrf.md)
- [Training, Labs, Videos, OSCP, AD, and Red Team](resource-indexes/training-labs-videos.md)
- [WAF, CORS, CSRF, Smuggling, and Web Platform Bypass](resource-indexes/web-platform-bypass.md)
- [XSS and Client-Side](resource-indexes/xss-client-side.md)
- [XXE, LFI, and File Read](resource-indexes/xxe-lfi-file-read.md)

## Exhaustive Resource Preservation

`all-source-resources.md` is intentionally exhaustive. It lists every external URL extracted from the original ZIP export, grouped by the raw source folder that contained it. Curated pages highlight the best resources; the exhaustive page proves nothing was silently dropped.

For browsing, start with [Resource Map](../RESOURCE-MAP.md). For audit, use [All Source Resources](all-source-resources.md).
